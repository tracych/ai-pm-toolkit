#!/usr/bin/env bash
# Smoke test for ai-redteam-prompts plugin structure.
...
SKILL_FILE="$PLUGIN_DIR/skills/ai-redteam-prompts/SKILL.md"
CMD_FILE="$PLUGIN_DIR/commands/ai-redteam-prompts.md"
CATEGORIES_FILE="$PLUGIN_DIR/assets/attack-categories.md"
...
# 6. Attack-categories file exists.
[ -f "$CATEGORIES_FILE" ] || fail "attack-categories file not found at $CATEGORIES_FILE"

# 7. attack-categories.md has exactly 8 numbered items (lines starting with N.).
numbered_count="$(grep -c '^[0-9]\.' "$CATEGORIES_FILE" || true)"
[ "$numbered_count" -eq 8 ] || fail "attack-categories.md must have exactly 8 numbered items, found $numbered_count"
