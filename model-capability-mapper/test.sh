#!/usr/bin/env bash
# Smoke test for model-capability-mapper plugin structure.
...
SKILL_FILE="$PLUGIN_DIR/skills/model-capability-mapper/SKILL.md"
CMD_FILE="$PLUGIN_DIR/commands/model-capability-mapper.md"
FAMILIES_FILE="$PLUGIN_DIR/assets/model-families.md"
...
# 6. assets/model-families.md exists.
[ -f "$FAMILIES_FILE" ] || fail "model-families.md not found at $FAMILIES_FILE"

# 7. model-families.md has exactly 10 numbered families (lines starting with N.).
families_count="$(grep -c '^[0-9]\{1,2\}\.[[:space:]]' "$FAMILIES_FILE" || true)"
[ "$families_count" -eq 10 ] || fail "model-families.md must have exactly 10 numbered families, found $families_count"

# 8. example-output.md exists.
[ -f "$EXAMPLE_FILE" ] || fail "example-output.md not found at $EXAMPLE_FILE"
